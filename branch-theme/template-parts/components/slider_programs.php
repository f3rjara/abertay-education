<?php
  $sub_slider_programs = get_sub_field('sub_slider_programs');
  /* CONFIGURATION SECTION */
  $hidden_section = $sub_slider_programs['visible_section'] ? 'd-none' : '';
  $id_section = $sub_slider_programs['id_section'];
  $class_custom_section = $sub_slider_programs['class_custom_section'];
...
<section  class="section-slider-programs py-abertay <?= $hidden_section ?> <?= $class_custom_section ?>" 
  ...
  <div class="container">
      ...
      <div class="col-12 col-lg-7 px-lg-2">
        <?php if (strlen($sub_slider_programs['caption_text']) > 0) : ?>
          <p class="caption-text mt-4 text-center text-p2 mb-4">
            <?= $sub_slider_programs['caption_text'] ?>
          </p>
        <?php endif; ?>
      </div>
    </div>
    ...
    <div class="row row-splide">
      <div  class="col-12 splide splide-cards-programm" role="group" 
            data-splide='{ "type":"loop", "pauseOnFocus":true, "arrows":false, "pagination":true }'>
        <div class="splide__track">
          <ul class="splide__list">
            <?php while ( $the_query->have_posts() ) : $the_query->the_post(); ?>
              <li class="splide__slide"><div class="h-100 m-2 d-flex justify-content-center">
    ...
    <?php $link = $sub_slider_programs['act_button'];  if( $link ): 
          $link_url = $link['url'];
